...
- **Safety abstraction** offers internal profiles (`strict`, `balanced`, `permissive`, `custom`) while mapping only documented provider controls. Gemini supports per-category thresholds such as `OFF`, `BLOCK_NONE`, `BLOCK_ONLY_HIGH`, `BLOCK_MEDIUM_AND_ABOVE`, and `BLOCK_LOW_AND_ABOVE`; OpenAI safety remains provider/model enforced and this project does not invent disabling flags.
...
curl -X POST http://localhost:8000/sessions \
  ...
  -d '{"provider":"openai","model":"gpt-image-1","default_safety_profile":"balanced"}'
```
...
```bash
image-service create-session --provider openai --model gpt-image-1
...
OpenAI and Gemini safety controls are not equivalent. Gemini exposes per-request `SafetySetting` category/threshold controls documented by Google, and responses can include prompt feedback and candidate safety ratings. OpenAI image generation through Responses API is isolated in the OpenAI adapter and exposes capability notes for unsupported safety controls. This service never claims to disable provider-enforced policy and records block/failure metadata instead of hiding it.
